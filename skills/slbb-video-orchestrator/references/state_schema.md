...
如果本 run 是长剧本地视频/录屏/字幕启动，先用 `slbb-video-source` 生成 `artifacts/_source/source_manifest.json` 和 `artifacts/_source/source_brief.md`。本地视频/直链视频必须再用 `slbb-video-ingest` 生成 `artifacts/_audit/video_ingest/`。
